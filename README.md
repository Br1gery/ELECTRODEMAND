...
В данной работе был исследован временной ряд [`PDB Load History`](https://www.kaggle.com/datasets/ashfakyeafi/pbd-load-history?select=PDB_Load_History.csv), отражающий почасовой уровень электрической нагрузки. Ряд имеет достаточно большой объем наблюдений и охватывает длительный период, что позволяет анализировать не только отдельные значения спроса, но и устойчивые закономерности во времени.

На этапе предварительного анализа было установлено, что временной ряд обладает выраженной сезонной структурой. В нем заметны суточные и недельные колебания, а также отдельные периоды повышенной и пониженной нагрузки.

На первом этапе была выполнена подготовка данных: проверены пропуски и дубликаты, изучено распределение целевой переменной и основные временные закономерности. Этот этап показал, что данные пригодны для дальнейшего моделирования, но требуют учета сезонной структуры и возможных резких отклонений.

На этапе статистического моделирования лучше всего показала себя модель `MSTL`. Она оказалась наиболее подходящей среди статистических методов, так как явно учитывает несколько сезонностей и хорошо описывает регулярный профиль нагрузки. При этом остатки модели сохранили автокорреляцию, поэтому статистический подход не полностью объясняет все изменения ряда.

На этапе data-driven моделирования сравнивались методы машинного обучения и нейросетевые модели. Лучший результат по RMSE показал RandomForest, а нейросетевые модели NBEATS и NHITS оказались близки к нему по качеству. Это показывает, что для данного временного ряда хорошо работают модели, использующие лаговые и календарные признаки. Линейные модели, наоборот, показали более слабое качество, так как простая линейная зависимость не смогла достаточно точно описать сложную сезонную структуру нагрузки.

Отдельно были рассмотрены методы обнаружения аномалий. `STL robust z-score` хорошо находил искусственно добавленные аномалии, но давал слишком много ложных срабатываний. `IsolationForest` оказался слишком консервативным и пропустил большую часть внедренных аномалий. Наиболее сбалансированным вариантом среди проверенных методов стал `LocalOutlierFactor`, так как он обнаружил большую часть аномалий и при этом дал меньше ложных срабатываний, чем STL-подход.

Итоговый вывод: для данного датасета наиболее надежным базовым решением оказался статистический пайплайн `MSTL + Holt`, потому что ряд имеет регулярную сезонную структуру, выраженные суточные и недельные закономерности, а также хорошо описывается через разложение сезонности. Data-driven методы, включая `RandomForest`, `NBEATS` и `NHITS`, полезны как часть исследования и проверки альтернатив: они могут лучше отрабатывать отдельные пики и сложные нелинейные зависимости, но не дали статистически значимого преимущества, а также требуют кратно больше времени и ресурсов по сравнению со статистическим подходом.