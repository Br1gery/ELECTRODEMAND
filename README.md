...
В проекте был исследован временной ряд [PDB Load History](https://www.kaggle.com/datasets/ashfakyeafi/pbd-load-history?select=PDB_Load_History.csv), содержащий 103 776 почасовых наблюдений за период с марта 2003 года по декабрь 2014 года. Целью работы было подготовить данные, провести EDA, сравнить статистические, ML и DL методы прогнозирования, рассмотреть методы поиска аномалий и собрать итоговый пайплайн для задачи временного ряда.

На этапе EDA было установлено, что ряд не содержит пропусков и дубликатов, но имеет выраженную сезонную структуру. В данных хорошо заметны суточные и недельные колебания, а также периоды повышенной и пониженной нагрузки. ADF-тест формально показал стационарность ряда, однако из-за сильной сезонности при моделировании всё равно необходимо учитывать календарные и лаговые зависимости.

Сравнение статистических методов показало, что лучшей моделью стала `MSTL`: она получила RMSE = 1115.48, MAE = 867.49 и MAPE = 6.257%. Это объясняется тем, что модель хорошо работает с несколькими сезонностями и лучше описывает регулярный профиль нагрузки.

Среди data-driven методов лучший результат по RMSE показал `RandomForest` — RMSE = 1095.87. Нейросетевые модели `NBEATS` и `NHITS` оказались близки к нему по качеству, а линейные модели заметно уступили. В задаче поиска аномалий `STL robust z-score` хорошо находил выбросы, но давал много ложных срабатываний, `IsolationForest` оказался слишком консервативным, а наиболее сбалансированным методом стал `LocalOutlierFactor`.

В итоговом пайплайне сравнивались `MSTL + Holt` и `RandomForest recursive` на holdout-периоде 504 часа. `RandomForest recursive` немного лучше показал себя по RMSE — 1127.62, то есть лучше отрабатывал отдельные крупные отклонения. `MSTL + Holt` оказался лучше по MAE = 783.81, WAPE = 5.479% и скорости работы: 3.33 сек против 8.38 сек. Статистические тесты не подтвердили значимого превосходства одного подхода над другим.

Итоговый вывод: для данного датасета наиболее надежным базовым решением оказался статистический пайплайн `MSTL + Holt`, потому что ряд регулярный, имеет выраженные суточные и недельные сезонные закономерности и хорошо описывается через разложение сезонности. ML и DL методы полезны как часть исследования и проверки альтернатив, особенно для отдельных пиков и нелинейных зависимостей, но они не дали статистически значимого преимущества.